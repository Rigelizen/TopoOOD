...
@torch.no_grad()
def evaluate_detect(model, dataset_ind, dataset_ood, criterion, eval_func, args, device, cluster_centers ,return_score=False,k=0):
    model.eval()
    if isinstance(model, Mahalanobis) or isinstance(model, KNN):
        test_ind_score = model.detect(dataset_ind, dataset_ind.splits['train'], dataset_ind, dataset_ind.splits['test'], 
                                      device, args)
    elif isinstance(model, TopoOOD):
        test_ind_score = model.detect(dataset_ind, dataset_ind.splits['test'], device, args, k=k, t=cluster_centers)
    else:
    ...
    if isinstance(dataset_ood, list):
        ...
        for d in dataset_ood:
            if isinstance(model, Mahalanobis) or isinstance(model, KNN):
                test_ood_score = model.detect(dataset_ind, dataset_ind.splits['train'], d, 
                                              d.node_idx, device, args).cpu()
            elif isinstance(model, TopoOOD):
                test_ood_score = model.detect(d, d.node_idx, device, args, k=k, t=cluster_centers)
            else:
        ...
        if isinstance(model, Mahalanobis) or isinstance(model, KNN):
            ...
            test_ood_score = model.detect(dataset_ood, dataset_ood.node_idx, device, args, k=k, t=cluster_centers)
        else:
