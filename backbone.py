...
class GCN(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels, num_layers=2,
                 dropout=0.5, save_mem=True, use_bn=True, use_aux=False, use_aux1=False):
        ...
        if use_aux:
            ...
            #     nn.ReLU(),
            #     # GCNConv(out_channels, 2, cached=not save_mem),
            #     nn.Linear(out_channels, 2)
            # )
            self.aux_conv = GCNConv(out_channels, 2, cached=not save_mem)

            self.aux_layer = nn.Sequential(
                nn.BatchNorm1d(out_channels)
            )

        self.aux_layer_1 = None
        if use_aux1:
            self.aux_layer_1 = nn.Sequential(
                nn.BatchNorm1d(out_channels),
                nn.ReLU(),
                # GCNConv(out_channels, 2, cached=not save_mem),
    ...
    def get_aux_output(self, x, edge_index):
...
class GNNTranAsap(torch.nn.Module):
    def __init__(self, num_layers, hidden, num_features, ratio=0.8, dropout=0):
        super().__init__()
        self.projx = Linear(num_features, hidden)
        self.convs = torch.nn.ModuleList()
        self.pools = torch.nn.ModuleList()
        self.convs.extend([
            TransformerConv(hidden, hidden, heads = 8, concat = False, dropout = 0) #, edge_dim = dataset.num_edge_features
    ...
    def forward(self, data):
        ...
        noise_y = data.noise_y.float()
        edge_weight = None   # data.edge_attr.float()
        x = F.relu(self.projx(x))
        ...
        x = self.jump(xs)
        # x = F.relu(self.lin1(x))
        x = F.relu(self.lin1(x + noise_y.unsqueeze(-1)))
