    ...
    def print_statistics(self, run=None):
        if run is not None:
            ...
            for r in result:
                ...
                argmin = valid_loss.argmin().item()
                score_val = test_score[argmin].item()
                ood_result_val = []
                for k in range(ood_te_num):
                    auroc_val = ood_result[argmin, k*3].item()
                    aupr_val = ood_result[argmin, k*3+1].item()
                    fpr_val = ood_result[argmin, k*3+2].item()
...
def save_result(results, args, suffix):

    if args.dataset in ('cora', 'amazon-photo', 'coauthor-cs','amazon-computer','coauthor-physics'):
        if args.ood_type == 'structure':
            filename = f'results/{args.dataset}-{args.ood_type}-{suffix}.csv'
        else:
            filename = f'results/{args.dataset}-{args.ood_type}.csv'
    elif args.dataset == 'arxiv' and args.ood_type == 'label':
        filename = f'results/{args.dataset}-{args.ood_type}.csv'
    else:
        filename = f'results/{args.dataset}-{suffix}.csv'
    ...
    if args.method == 'gnnsafe':
        ...
        if args.use_prop:
            name = 'knn++' if args.use_reg else 'knn'
        else:
            name = 'knn++ w/o prop' if args.use_reg else 'knn w/o prop'
    elif args.method == 'bc':
        if args.use_prop:
            name = args.method
        else:
    ...
    with open(f"{filename}", 'a+') as write_obj:
        if 'knn' in name: 
            write_obj.write(f"{name} {args.backbone} K={args.K} deng={args.deng_level} knn_k={args.knn} lr={args.lr} lamda={args.lamda}\n")
        elif 'bc' in name:
            if args.dataset in ['cora', 'amazon-photo', 'coauthor-cs']:
                write_obj.write(f"{name} {args.backbone} K={args.K} K_d={args.K_d} lr={args.lr} lamda={args.lamda} lamda2={args.lamda2} deng_level={args.deng_level} m_in={args.m_in} m_out={args.m_out}\n")
            else:
                write_obj.write(f"{name} {args.backbone} K={args.K} K_d={args.K_d} lr={args.lr} lamda={args.lamda} lamda2={args.lamda2} deng_level={args.deng_level} m_in={args.m_in} m_out={args.m_out}\n")
        else:
            write_obj.write(f"{name} {args.backbone} K={args.K} lr={args.lr}\n")
